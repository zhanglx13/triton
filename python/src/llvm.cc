...
std::string translateLLVMIRToASM(
    ...
    bool disableSched = false, bool forceMfmaAgprForm = false) {
  using namespace mlir;
...
void init_triton_llvm(py::module &&m) {
  ...
  m.def(
      ...
      [](std::string llvmIR, std::string triple, std::string proc,
         ...
         bool enable_fp_fusion, bool isObject, bool canonicalizeGEP,
         bool disableSched, bool forceMfmaAgprForm) -> py::object {
        std::string obj;
        {
          ...
          obj = translateLLVMIRToASM(*module, triple, proc, features, flags,
                                     ...
                                     disableSched, forceMfmaAgprForm);
        }
        if (isObject)
          return py::bytes(obj);
        else
